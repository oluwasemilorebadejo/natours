...
exports.restrictTo =
  (...roles) =>
  // returns array ['admin', 'lead-guide']
  (req, res, next) => {
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  await user.save({ validateBeforeSave: false }); // from the createpasswordresettoken func, we only modified passwordresetexpires, hence we now save it
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  const user = await User.findOne({
    ...
    passwordResetExpires: { $gt: Date.now() }, // checks if token has expired too, returns user only if query is true
  });
  // if token hasnt expired and user exists, set the new password
  if (!user) {
  ...
  await user.save(); // save the new values into the document
  // update changepasswordat property of the user -> this was done in the user model
  // log the user in i.e send jwt
  createSendToken(user, 200, res);
});
...
exports.updatePassword = catchAsync(async (req, res, next) => {
  ...
  if (!(await user.correctPassword(req.body.passwordCurrent, user.password))) {
    return next(new AppError("your current password is wrong password", 401));
  }
  // if previous step passes, update password
  user.password = req.body.password;
  user.passwordConfirm = req.body.passwordConfirm;
  await user.save(); // updateById wont work as intended
  // log user in ie send jwt
  createSendToken(user, 200, res);
});
